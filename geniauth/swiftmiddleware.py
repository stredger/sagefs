...
#
# Big time ripoff (fork if you want to get technical :P (... except i didnt fork it))
#  of the original Swift TempAuth system. Only changes are in the handle_get_token
#  method and a few names.
...
class GeniAuth(object):
  ...
  def __init__(self, app, conf):
    ...
    # make sure we have a reseller_prefix. the prefix is 
    #  just to make sure we avoid conflict with other auth 
    #  services which may run somewhere else in the pipeline
    ...
    # add dict of superusers in the conf file, these will be checked before
    #  we auth with GENI. This is so we can have an admin user, and also
    #  keep the swift tempauth functionality. Note: not all users in here
    #  have superuser like privaleges!
    self.superusers = {}
  ...
  def handle_get_token(self, req):
    ...
    if account_user in self.superusers:
      if self.superusers[account_user]['key'] != key:
        self.logger.increment('token_denied')
        return HTTPUnauthorized(request=req, headers=
                    {'Www-Authenticate':
                     'Swift realm="%s"' % account})
